...
{
    Logger logger = LoggerFactory.getLogger( VelocimacroFactory.class );
...
//    private final LogDisplayWrapper log;
    ...
    {
        this.rsvc = rsvc;
//        this.log = new LogDisplayWrapper( "Velocimacro : ", rsvc.getBoolean(RuntimeConstants.VM_MESSAGES_ON, true));
    ...
    public void initVelocimacro()
    {
        ...
        synchronized(this)
        {
            logger.trace("initialization starting.");
             ...
             */
             ...
             if (libfiles == null)
             {
                 logger.debug("\"" + RuntimeConstants.VM_LIBRARY +
                 ...
                 {
                     logger.debug("Default library not found.");
                 }
             }
             ...
             if(libfiles != null)
             {
                 ...
                 for(int i = 0, is = macroLibVec.size(); i < is; i++)
                 {
                     ...
                     if (StringUtils.isNotEmpty(lib))
                     {
                         ...
                         logger.debug("adding VMs from VM library : " + lib);
                         ...
                         logger.trace("VM library registration complete.");
            ...
            if (!rsvc.getBoolean( RuntimeConstants.VM_PERM_ALLOW_INLINE, true))
            {
                ...
                logger.debug("allowInline = false : VMs can NOT be defined inline in templates");
            }
            else
            {
                logger.debug("allowInline = true : VMs can be defined inline in templates");
            }
                 ...
                 RuntimeConstants.VM_PERM_ALLOW_INLINE_REPLACE_GLOBAL, false))
            {
                ...
                logger.debug("allowInlineToOverride = true : VMs defined inline may replace previous VM definitions");
            }
            else
            {
                logger.debug("allowInlineToOverride = false : VMs defined inline may NOT replace previous VM definitions");
            }
            ...
            {
                logger.debug("allowInlineLocal = true : VMs defined inline will be local to their defining template only.");
            }
            else
            {
                logger.debug("allowInlineLocal = false : VMs defined inline will be global in scope if allowed.");
            }
            ...
            if (getAutoload())
            {
                logger.debug("autoload on : VM system will automatically reload global library macros");
            }
            else
            {
                logger.debug("autoload off : VM system will not automatically reload global library macros");
            }

            logger.trace("Velocimacro : initialization complete.");
        }
    }
            ...
            String argArray[], String sourceTemplate)
    {
        ...
        if (logger.isDebugEnabled())
        {
            logger.debug("added VM "+name+": source="+sourceTemplate);
        }
        return true;
    }
    ...
    private synchronized boolean canAddVelocimacro(String name, String sourceTemplate)
    {
        ...
        if (!templateLocal)
        {
            ...
            if (!replaceAllowed && isVelocimacro(name, sourceTemplate))
            {
                 ...
                 */ 
                if (logger.isDebugEnabled())
                    logger.debug("VM addition rejected : "+name+" : inline not allowed to replace existing VM");
                return false;
            }
        }
                ...
                if (lib != null)
                {
                    try
                    {
                        ...
                        if (tw != null)
                        {
                            ...
                            if (ft > tt)
                            {
                                logger.debug("auto-reloading VMs from VM library : " + lib);
