...
{
    Logger logger = LoggerFactory.getLogger( URLResourceLoader.class );
    ...
    {
        logger.trace("URLResourceLoader : initialization starting.");
        ...
        if (logger.isDebugEnabled())
        {
            ...
            {
                logger.debug("URLResourceLoader : adding root '" + roots[i] + "'");
            }
        }
        ...
        if (timeout > 0)
        {
            try
            {
                ...
                logger.debug("URLResourceLoader : timeout set to "+timeout);
            }
            catch (NoSuchMethodException nsme)
            {
                logger.debug("URLResourceLoader : Java 1.5+ is required to customize timeout!", nsme);
                timeout = -1;
            }
        }
        ...
        logger.trace("URLResourceLoader : initialization complete.");
    }
        ...
        throws ResourceNotFoundException
    {
        ...
        for(int i=0; i < roots.length; i++)
        {
            try
            {
                ...
                if (inputStream != null)
                {
                    if (logger.isDebugEnabled()) logger.debug("URLResourceLoader: Found '"+name+"' at '"+roots[i]+"'");
            ...
            {
                if (logger.isDebugEnabled()) logger.debug("URLResourceLoader: Exception when looking for '"+name+"' at '"+roots[i]+"'", ioe);
