...
{
    Logger logger = LoggerFactory.getLogger( DataSourceResourceLoader.class );
    ...
    public void init(ExtendedProperties configuration)
    {
        ...
        if (dataSource != null)
        {
            ...
            {
                logger.debug("DataSourceResourceLoader: using dataSource instance with table \""
                          + tableName + "\"");
                logger.debug("DataSourceResourceLoader: using columns \""
                          + keyColumn + "\", \"" + templateColumn + "\" and \""
                          + timestampColumn + "\"");
            }

            logger.trace("DataSourceResourceLoader initialized.");
        }
        else if (dataSourceName != null)
        {
            if (logger.isDebugEnabled())
            {
                logger.debug("DataSourceResourceLoader: using \"" + dataSourceName
        ...
        throws ResourceNotFoundException
    {
        ...
        try
        {
            ...
            if (rs.next())
            {
                InputStream stream = rs.getAsciiStream( templateColumn );
                if (stream == null)
                {
