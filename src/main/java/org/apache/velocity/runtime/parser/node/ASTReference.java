...
{
    Logger logger = LoggerFactory.getLogger( ASTReference.class );
            ...
            for (int i = 0; i < numChildren; i++)
            {
                if (strictRef && result == null)
                {
                    ...
                    throw new VelocityException("Attempted to access '"  
                        ...
                        + Formatter.formatFileString( uberInfo.getTemplateName(),
            ...
            MethodInvocationException
    {
        ...
        {          
            if (value instanceof Renderable)
            {
                ...
                catch(RuntimeException e)
                {
                    ...
                    logger.error("Exception rendering "
                        + ((renderable instanceof Reference)? "block ":"Renderable ")
                        + rootString + " at " + Formatter.formatFileString(this));
                    throw e;
                }
            }
        ...
        {          
            if (strictRef)
            {
                ...
                {
                  logger.error("Prepend the reference with '$!' e.g., $!" + literal().substring(1)
            ...
            if (logOnNull && referenceType != QUIET_REFERENCE && logger.isDebugEnabled())
            {
                logger.debug("Null reference [template '" + getTemplateName()
        ...
        else if (toStringNullCheck)
        {
            ...
            catch(Exception e)
            {
                throw new VelocityException("Reference evaluation threw an exception at " 
                    + Formatter.formatFileString(this), e);
            }
        }
        else
        {
      ...
      throws MethodInvocationException
    {
        ...
        if (result == null)
        {
            String msg = "reference set is not a valid reference at "
                    + Formatter.formatFileString(uberInfo);
            logger.error(msg);
            return false;
        }
            ...
            if (method == null)
            {
                ...
                if (strictRef)
                {
                    throw new VelocityException(
                        ...
                        + "' at " + Formatter.formatFileString(astIndex));
                }
                return false;
            }
        ...
        catch(Exception e)
        {
            ...
            String msg = "ASTReference setValue() : exception : " + e
                          + " template at " + Formatter.formatFileString(uberInfo);
            logger.error(msg, e);
    ...
    public Object getVariableValue(Context context, String variable) throws MethodInvocationException
    {
        ...
        {
            logger.error("Exception calling reference $" + variable + " at "
                      + Formatter.formatFileString(uberInfo));
            throw e;
        }
        ...
        if (strictRef && obj == null)
        {
          ...
          {
              logger.error("Variable $" + variable + " has not been set at "
                        + Formatter.formatFileString(uberInfo));
