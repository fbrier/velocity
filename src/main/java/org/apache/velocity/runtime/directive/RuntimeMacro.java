...
{
    Logger logger = LoggerFactory.getLogger( RuntimeMacro.class );
                     ...
                     Node node)
    {
        ...
        for (int n=0; n < node.jjtGetNumChildren(); n++)
        {
            ...
            if (child.getType() == ParserTreeConstants.JJTWORD)
            {
                ...
                + "' in macro #" + macroName + " at " + Formatter.formatFileString( child );
        ...
        if (vmProxy != null)
        {
            ...
            catch (TemplateInitException die)
            {
                throw new ParseErrorException(die.getMessage() + " at "
                    + Formatter.formatFileString(node), new Info(node));
            }
            ...
            catch (RuntimeException e)
            {
                 ...
                 */
                logger.error( "Exception in macro #" + macroName + " called at " +
                        Formatter.formatFileString( node ) );
                throw e;
            }
            catch (IOException e)
            {
