...
public abstract class Block extends Directive
{
    ...
    {
        Logger logger = LoggerFactory.getLogger( Reference.class );
        ...
        public boolean render(InternalContextAdapter context, Writer writer)
        {
            ...
            if (depth > parent.maxDepth)
            {
                ...
                if ( logger.isDebugEnabled() )
                    logger.debug("Max recursion depth reached for " + parent.id(context)
                        + " at " + Formatter.formatFileString( parent ));
                depth--;
                return false;
            }
            else
