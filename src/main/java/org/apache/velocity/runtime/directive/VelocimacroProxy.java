...
{
    Logger logger = LoggerFactory.getLogger( VelocimacroProxy.class );
            ...
            throws IOException, MethodInvocationException, MacroOverflowException
    {
        ...
        if (maxCallDepth > 0 && maxCallDepth == vmc.getCurrentMacroCallDepth())
        {
            ...
            out.append(" at " + Formatter.formatFileString( this ));
    ...
    public void init(RuntimeServices rs)
    {
        ...
        if (localContextScope && logger.isWarnEnabled())
        {
            ...
            if (alreadyWarned == null)
            {
                ...
                logger.warn("The "+RuntimeConstants.VM_CONTEXT_LOCALSCOPE+
    ...
    public void checkArgs(InternalContextAdapter context, Node node, boolean hasBody)
    {
        ...
        if (getNumArgs() != i)
        {
            ...
            {
                logger.debug(buildErrorMsg(node, i));
                return;
            }
        }
    }
