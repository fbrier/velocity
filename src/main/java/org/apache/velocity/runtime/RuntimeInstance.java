...
{
    Logger logger = LoggerFactory.getLogger( RuntimeInstance.class );
    ...
    public synchronized void init()
    {
        ...
        {
            logger.debug("Initializing Velocity, Calling init()...");
            initializing = true;

            logger.trace("*******************************************************************");
            logger.debug("Starting Apache Velocity v@build.version@ (compiled: @build.time@)");
            logger.trace("RuntimeInstance initializing.");
            ...
            logger.trace("RuntimeInstance successfully initialized.");
    ...
    /**
     * Init or die! (with some logger.help, of course)
     */
    private void requireInitialization()
    {
        if (!initialized)
        {
            ...
            {
                logger.error("Could not auto-initialize Velocity", e);
    ...
    private void initializeDirectives()
    {
        ...
        while (directiveClasses.hasMoreElements())
        {
            ...
            logger.debug("Loaded System Directive: " + directiveClass);
        }
        ...
        for( int i = 0; i < userdirective.length; i++)
        {
            ...
            if (logger.isDebugEnabled())
            {
                logger.debug("Loaded User Directive: " + userdirective[i]);
            }
        }
        ...
        throws ParseException
    {
        ...
        if (parser == null)
        {
            ...
            {
                logger.info("Runtime : ran out of parsers. Creating a new one. "
